...
const getVisual = (catName = '', typeName = '') => {
  ...
  if (t.includes('abs') || c.includes('ирмэг')) return { bg: 'linear-gradient(135deg,#fdf2f8,#f5d0fe)', color: '#701a75', p: 'band' };
...
export default function MaterialsPagePublic() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      const uniqueCats = rawCats.reduce((acc: any[], cat: any) => {
        if (acc.find((a: any) => a.name === cat.name)) return acc;
        ...
        const uniqueTypes = cat.material_types?.reduce((tacc: any[], t: any) => {
          if (tacc.find((ta: any) => ta.name === t.name)) return tacc;
  ...
  if (activeCat !== 'all') filtered = filtered.filter(m => m.material_types?.material_categories?.id === activeCat);
  if (activeType) filtered = filtered.filter(m => m.material_types?.id === activeType);
  if (search) {
  ...
  const goDetail = (id: number) => router.push(`/materials-page/${id}`);

  const handleAddToCart = (e: React.MouseEvent, m: Material) => {
    e.stopPropagation();
    addToCart({
      id: m.id, code: m.code, name: m.name, unit: m.unit, price: m.price,
      image_url: m.material_images?.find(i => i.is_primary)?.url || m.material_images?.[0]?.url || m.image_url,
      type_name: m.material_types?.name,
    });
    setAddedIds(prev => {
      const s = new Set(prev); s.add(m.id);
      setTimeout(() => setAddedIds(p => { const n = new Set(p); n.delete(m.id); return n; }), 1500);
      return s;
    });
  };
  ...
  const Skel = () => (
    <div style={{ background: 'white', border: '1.5px solid #f3f4f6', borderRadius: 16, overflow: 'hidden' }}>
      ...
      <div style={{ padding: 14 }}>
        {[50, 80, 40, 60].map((w, i) => (
          <div key={i} style={{ height: i === 1 ? 13 : 10, width: `${w}%`, background: 'linear-gradient(90deg,#f1f5f9 25%,#e2e8f0 50%,#f1f5f9 75%)', backgroundSize: '200% 100%', animation: 'shimmer 1.4s infinite', borderRadius: 5, marginBottom: 8 }} />
        ))}
      </div>
    </div>
  );
    ...
    <>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        .cart-btn{display:flex;align-items:center;gap:7px;background:#1c1917;color:white;border:none;border-radius:11px;padding:9px 18px;font-size:13px;font-weight:700;cursor:pointer;font-family:inherit;transition:all 0.2s}
        ...
        .cat-tabs{display:flex;gap:6px;margin-bottom:8px;overflow-x:auto;flex-wrap:nowrap;padding-bottom:4px;scrollbar-width:none}
        ...
        .type-row{display:flex;gap:6px;margin-bottom:16px;overflow-x:auto;flex-wrap:nowrap;scrollbar-width:none}
        ...
        @media(max-width:768px){.controls{flex-wrap:wrap}.main{padding:20px 16px 40px}.topnav{padding:0 16px}.prod-grid{grid-template-columns:repeat(2,1fr);gap:12px}.nav-brand-name{display:none}}
        @media(max-width:480px){.prod-grid{grid-template-columns:repeat(2,1fr);gap:10px}.nav-title{font-size:12px}.cart-btn{padding:8px 12px}}
      `}</style>
      ...
      <nav className="topnav">
        ...
        <div className="nav-r">
          <button className="cart-btn" onClick={() => router.push('/checkout')}>
            🛒 Сагс
          ...
          {authUser ? (
            ...
            <button onClick={() => router.push('/auth/login')}
              style={{ background: 'white', color: '#0f172a', border: '1.5px solid #e2e8f0', borderRadius: 10, padding: '9px 16px', fontSize: 13, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>
              Нэвтрэх
            </button>
          )}
        </div>
      ...
      <div className="main">
        ...
        <div className="controls">
          <div className="search-box">
            ...
            <input placeholder="Нэр, код, төрлөөр хайх..." value={search} onChange={e => setSearch(e.target.value)} />
            {search && (
        ...
        <div className="cat-tabs">
          <button className={`ct ${activeCat === 'all' ? 'on' : ''}`} onClick={() => { setActiveCat('all'); setActiveType(null); }}>
            Бүгд ({materials.length})
          </button>
          {categories.map(cat => (
            <button key={cat.id} className={`ct ${activeCat === cat.id ? 'on' : ''}`}
        ...
        {activeCat !== 'all' && (() => {
          ...
          return (
            <div className="type-row">
              <button className={`tp ${!activeType ? 'on' : ''}`} onClick={() => setActiveType(null)}>Бүгд</button>
              {cat.material_types.map(t => (
                <button key={t.id} className={`tp ${activeType === t.id ? 'on' : ''}`}
        ...
        <div className="result-bar">
          <span className="result-count">
            <strong>{filtered.length}</strong> материал олдлоо{search && ` — "${search}"`}
          </span>
        ...
        {/* MATERIALS */}
        {loading ? (
          <div className="prod-grid">{Array.from({ length: 8 }).map((_, i) => <Skel key={i} />)}</div>
        ) : filtered.length === 0 ? (
          <div className="empty">
          ...
          <div className="prod-grid">
            {filtered.map(m => {
              ...
              return (
                <div key={m.id} className="prod-card" onClick={() => goDetail(m.id)}>
                  <MatImg m={m} />
                    ...
                    <div className="prod-bottom">
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                        ...
                        <button className={`add-btn ${isAdded ? 'added' : ''}`}
                          onClick={e => handleAddToCart(e, m)} title="Сагсанд нэмэх">
                          {isAdded ? '✓' : '🛒'}
                        </button>
                      </div>
                    </div>
          ...
          <div>
            {filtered.map(m => {
              const isAdded = addedIds.has(m.id);
              return (
                <div key={m.id} className="list-row" onClick={() => goDetail(m.id)}>
                  <div className="list-img"><MatImg m={m} h={60} /></div>
                  ...
                  <button className={`list-add-btn ${isAdded ? 'added' : ''}`}
